    ...
    with open(output_file, 'w') as f:
        ...
        for epoch in range(max_epoch):
            for batch_idx, (inputs, labels) in enumerate(train_loader):
                ...
                if len(current_segment) == 20 and batch_number >=200: #200 is the min no of batch before it applies stopping critreia
...
def main():
    ...
    ACCEPTABLE_PRUNE_PERCENTAGES = [0.985,0.99]
    ACCEPTABLE_BATCH_SIZES = [64, 1024]

    num_runs = 100

    for run_index in range(12,num_runs):
