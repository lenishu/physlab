...
# Define a simple convolutional neural network with pruning for CIFAR-100
class PrunedConvol(nn.Module):
    def __init__(self, prune_percentage=50.0, Prune_Layers='ALL'):
        super(PrunedConvol, self).__init__()
        # CIFAR-100 images are 3-channel (RGB)
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.relu = nn.ReLU()
        self.flatten = nn.Flatten()
        # CIFAR-100 images are 32x32, so flattened size is 32*32*32 channels
        self.fc1 = nn.Linear(32 * 32 * 32, 128, bias=False)
        self.fc2 = nn.Linear(128, 100, bias=False)  # Changed from 10 to 100
...
def main():
    ...
    # CIFAR-100 mean and std for normalization (RGB channels)
