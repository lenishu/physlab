...
# Define a simple convolutional neural network with pruning for CIFAR-10
class PrunedConvol(nn.Module):
    def __init__(self, prune_percentage=50.0, Prune_Layers='ALL'):
        ...
        # CIFAR-10 images are 32x32, so flattened size is 32*32*32 channels
        self.fc1 = nn.Linear(32 * 32 * 32, 128, bias=False)
...
def calculate_epochs_for_batch_size(batch_size):
    # These values can be adjusted as needed for CIFAR-10 training
    if batch_size == 64:
        return 5
...
def main():
    ...
    # CIFAR-10 mean and std for normalization (RGB channels)
    ...
    ACCEPTABLE_PRUNE_PERCENTAGES = [i / 100 for i in range(0, 10, 10)]
