...
class PrunedMLP(nn.Module):
    def __init__(self, prune_percentage=50.0, Prune_Layers='ALL'):
        ...
        # Flatten the 28x28 input images (FashionMNIST)
        self.flatten = nn.Flatten()
        # Define a single fully connected layer (input to output)
        self.fc1 = nn.Linear(28 * 28, 10, bias=False)  # Input to output (10 classes for FashionMNIST)
        self.relu = nn.ReLU()
...
def train(model, train_loader, test_loader, criterion, optimizer, fraction_of_epoch, device, output_file):
    with open(output_file, 'w') as f:
        ...
        #Creating a variable for max epoch
        max_epoch = 500
        for epoch in range(max_epoch):  # We are allowing up to 500 epochs, but will stop earlier if target_batches are processed
...
def main():
    ...
    # Load FashionMNIST dataset and create DataLoader
        ...
        for prune_layers in PRUNE_LAYERS_OPTIONS:
            ...
            for prune_percentage in ACCEPTABLE_PRUNE_PERCENTAGES:
                ...
                for batch_size in ACCEPTABLE_BATCH_SIZES:
                    ...
                    output_file = os.path.join(prune_layers_directory, prune_directory, batch_directory, f"slp_{prune_percentage}_{batch_size}_run_{run_index}.txt")
